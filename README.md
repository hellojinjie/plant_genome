...
- 🎯 Entry points to execute your program using `python -m <plant_genome>` or `$ plant_genome` with basic CLI argument parsing.
...
---
# plant_genome Flask Application

Awesome plant_genome created by hellojinjie
...
```bash
git clone https://github.com/hellojinjie/plant_genome plant_genome
cd plant_genome
make install
```
...
```bash
plant_genome create-db   # run once
plant_genome populate-db  # run once (optional)
plant_genome add-user -u admin -p 1234  # ads a user
plant_genome run
```
