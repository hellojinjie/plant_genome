...
original_author="hellojinjie"
original_name="plant_genome"
original_urlname="plant_genome"
original_description="Awesome plant_genome created by hellojinjie" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/plant_genome "${name}"
...
then
    ...
    echo "then run 'plant_genome' or 'python -m plant_genome'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'plant_genome' or 'python -m plant_genome' or 'poetry run plant_genome'"
fi
